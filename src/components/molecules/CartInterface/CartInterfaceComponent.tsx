...
import {
  ...
  Stock,
  ViewDeliveryOptions,
} from './CartInterfaceComponent.styles';

interface Props {
  product: productBody;
}

const CartInterfaceComponent = ({
  product: { price, discountPercentage, stock, brand, category },
}: Props) => {
  ...
  return (
    <CartInterface>
      ...
      <OffPromoPrice>
        Most shown price: <OffPrice>{calcPriceBeforeDiscount()}$</OffPrice>
      </OffPromoPrice>
      ...
      <SafetyInfoBlock>
        <InfoList>
          ...
          <InfoElement>
            <InfoImportant>Free</InfoImportant> shipping over 20 euro, sent by
            Superpupershop.xdd
          </InfoElement>
          <InfoElement>
            <InfoImportant>Free</InfoImportant> collection at a Superpupershop.xdd
            collection point is possible
          </InfoElement>
          <InfoElement>
            30 days&apos; reflection period and <InfoImportant>free</InfoImportant>{' '}
            returns
          </InfoElement>
      ...
      <CheckOtherProducts>
        Check other products from category {category}.
      </CheckOtherProducts>
      <DeliveryInfoBlock>
        <InfoList>
          <InfoElement>
            Delivered <Bold>today</Bold> (order Mon-Fri before 12:00, delivery between
            5:00 PM and 10:00 PM)
          </InfoElement>
          <InfoElement>
            Also at home in the <Bold>evening</Bold> during the week
          </InfoElement>
          <InfoElement>
            Also delivered on <Bold>Sunday</Bold> (order before Sat 11:59 PM)
          </InfoElement>
        </InfoList>
        <ViewDeliveryOptions>
